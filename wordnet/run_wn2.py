...
## extract whole synset

import json, numpy, collections
from progressbar import *

save_path_pref = 'dataset_wn/quac10_v6/'        # common prefix of processed documents  
data_path = "dataset/quac_context/QuAC10/train_v0.2.json"                              # clustered source 
data_t_path = "dataset/quac_context/QuAC10/val_v0.2.json"
# save_path_pref = 'dataset_wn/quac10_v6_small/'        # common prefix of processed documents  
    ...
    for paragraph in article['paragraphs']:
        for qas in paragraph['qas']:
            ...
            for word in words:
                ...
                best_wd_idx = 0
                best_wd_set = []
                for idx_s, s in enumerate(wd_set):
                ...
                # print(words_syn)
                for idx_s, s in enumerate(wd_set):
                    if idx_s==best_wd_idx:
                        best_wd_set = s_list
                        for s_wd in s_list: 
                            if s_wd !="be" and s_wd!="do" and s_wd!="have":
                                kg_list.append(s_wd)

                for best_wd_s in best_wd_set:
                    if best_wd_s not in q_kg.split() and best_wd_s!="be" and best_wd_s!="do" and best_wd_s!="have": 
                        q_kg = q_kg.replace(wd,wd + " " + best_wd_s)
...
print("train_miss : ", train_miss, " train_miss_t: ", train_miss_t)
