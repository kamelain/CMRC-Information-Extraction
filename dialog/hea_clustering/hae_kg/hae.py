...
tf.gfile.MakeDirs(FLAGS.cache_dir + FLAGS.dataset.lower())
...
cls_ids = tf.placeholder(tf.int32, shape=[None, FLAGS.max_seq_length], name='cls_ids')
kg_ids = tf.placeholder(tf.int32, shape=[None, FLAGS.max_seq_length], name='kg_ids')
...
with tf.Session() as sess:
    ...
    if FLAGS.do_train:
        ...
        for step, batch in enumerate(train_batches):
            ...
            print("fd['unique_ids']: ", len(fd['unique_ids']))
            print("fd['input_ids']: ", len(fd['input_ids']))
            print("fd['input_mask']: ", len(fd['input_mask']))
            print("fd['segment_ids']: ", len(fd['segment_ids']))
            print("fd['cls_ids']: ", len(fd['cls_ids']))
            print("fd['kg_ids']: ", len(fd['kg_ids']))
            print("fd['start_positions']: ", len(fd['start_positions']))
            print("fd['end_positions']: ", len(fd['end_positions']))
            print("fd['history_answer_marker']: ", len(fd['history_answer_marker']))

            try:
                _, train_summary, total_loss_res = sess.run([train_op, merged_summary_op, total_loss], \
                    feed_dict={\
                        unique_ids: fd['unique_ids'], \
                        input_ids: fd['input_ids'], \
                        input_mask: fd['input_mask'], \
                        segment_ids: fd['segment_ids'], \
                        cls_ids: fd['cls_ids'], \
                        kg_ids: fd['kg_ids'], \
                        start_positions: fd['start_positions'], \
                        end_positions: fd['end_positions'],  \
                ...
                for val_batch in val_batches:
                    ...
                    try:
                        ...
                        start_logits_res, end_logits_res, batch_total_loss = sess.run([start_logits, end_logits, total_loss], 
                                    ...
                                    input_mask: fd['input_mask'], segment_ids: fd['segment_ids'], cls_ids: fd['cls_ids'], kg_ids: fd['kg_ids'],
