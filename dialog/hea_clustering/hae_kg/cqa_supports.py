...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            kg_ids = [0] * len(input_ids)       # not use
            for idx in example.kg_idx:
                input_mask[idx+1] = 0           # [CLS] + query masking index 
                segment_ids[idx+1] = 2
            ...
            assert len(kg_ids) == max_seq_length
...
def convert_features_to_feed_dict(features):
    ...
    batch_segment_ids, batch_cls_ids, batch_kg_ids, batch_start_positions, batch_end_positions, batch_history_answer_marker = [], [], [], [], [], []
    ...
    for feature in features:
        ...
        try: 
            batch_kg_ids.append(feature.kg_ids)
        except:
            print("failed to add kg_ids")
    ...
    feed_dict = {'unique_ids': batch_unique_ids, 'input_ids': batch_input_ids, \
        'input_mask': batch_input_mask, 'segment_ids': batch_segment_ids, 'cls_ids': batch_cls_ids, 'kg_ids' : batch_kg_ids, \
