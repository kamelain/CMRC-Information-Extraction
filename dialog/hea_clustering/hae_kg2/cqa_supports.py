...
## WN or PLSA or PRGC ##        
        questions_wn = [item['question_prgc'] for item in entry['qas']]
        questions_wn_list =  [item['question_prgc_list'] for item in entry['qas']]

        # questions_wn = [item['question_plsa'] for item in entry['qas']]
        # questions_wn_list =  [item['question_plsa_list'] for item in entry['qas']]

        # questions_wn = [item['question_wn'] for item in entry['qas']]
        # questions_wn_list =  [item['question_wn_list'] for item in entry['qas']]
## WN or PLSA or PRGC ##

        qas = []
        for i, (question, answer, followup, yesno, question_wn, question_wn_list) in enumerate(zip(questions, answers, followups, yesnos, questions_wn, questions_wn_list)):
            ...
            qas.append({'id': question[1], 'question': question_with_histories, 'answers': [{'answer_start': answer[1], 'text': answer[0]}],
                        'history_answer_marker': history_answer_marker, 'metadata': metadata, 'question_wn': question_wn, 'question_wn_list': question_wn_list})
...
# Q source
            ...
            # (use wordnet)
            question_text = qa["question_wn"]
            question_new_text = ""
# Q source
            # KG: get synsets index
            kg_idx = []
            kg_list = qa["question_wn_list"]
...
# K-POS (no synonyms)
# K-BERT (with synonyms) 
                        # kg_idx_offset = len(kg_idx)//2   # count "Synonym" offset
                        # kg_idx.append(real_idx+kg_idx_offset)       # for + K-BERT (with relation), + entity
                        # kg_idx.append(real_idx+kg_idx_offset+1)     #                               + relation
    # # WN
    #                     question_new_text = question_new_text + " synonym"
    # # WN
    # # PLSA
                        # question_new_text = question_new_text + " relevance"
    # # PLSA  
...
# K-BERT (with synonyms)
                ...
                # if kpos in example.kg_idx: 
                #     if example.kg_idx.index(kpos)%2!=0 and kpos not in kg_done_list: 
...
# K-BERT (with synonyms)
            ...
            for i in range(doc_span.length):
                ...
                cls_ids.append(example.cls_label)
                kpos = kpos + 1
                kpos_ids.append(kpos)
            tokens.append("[SEP]")
            marker.append(0)
...
# MASK
            # KG mask, segment
            for idx in example.kg_idx:
                input_mask[idx] = 0             # [CLS] + query masking index 
# MASK
            ...
            while len(input_ids) < max_seq_length:
                ...
                cls_ids.append(0)
                marker.append(0)
# POS. EMB END with last index
            while len(kpos_ids) < max_seq_length: 
                kpos_ids.append(max_seq_length-1)
# POS. EMB END with last index
            assert len(input_ids) == max_seq_length
            assert len(input_mask) == max_seq_length
            assert len(segment_ids) == max_seq_length
            assert len(cls_ids) == max_seq_length
            assert len(kpos_ids) == max_seq_length
...
#                 tf.logging.info(
#                     "cls_ids: %s" % " ".join([str(x) for x in cls_ids]))
#                 tf.logging.info(
...
def convert_features_to_feed_dict(features):
    ...
    batch_segment_ids, batch_cls_ids, batch_kpos_ids, batch_start_positions, batch_end_positions, batch_history_answer_marker = [], [], [], [], [], []
    ...
    for feature in features:
        ...
        try: 
            ...
            print("failed to add cls_ids")
        try: 
            batch_kpos_ids.append(feature.kpos_ids)
        except:
            print("failed to add kpos_ids")
    ...
    feed_dict = {'unique_ids': batch_unique_ids, 'input_ids': batch_input_ids, 
              'input_mask': batch_input_mask, 'segment_ids': batch_segment_ids, 'cls_ids': batch_cls_ids, 'kpos_ids': batch_kpos_ids,
