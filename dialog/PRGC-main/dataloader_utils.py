...
class InputExample(object):
    ...
    def __init__(self, text, en_pair_list, re_list, rel2ens, id=None):
        self.text = text
    ...
    if data_sign == 'app':
        with open(data_dir / f'{data_sign}_triples.json', "r", encoding='utf-8') as f:
            ...
            for sample in data:
                text = sample['text']
                id = sample['id']
                example = InputExample(text=text, en_pair_list=[], re_list=[], rel2ens=defaultdict(list), id=id)
...
def get_tokens(example, tokenizer, max_text_len=512):
...
def convert(example, max_text_len, tokenizer, rel2idx, data_sign, ex_params):
    ...
    # token to id
    input_ids = tokenizer.convert_tokens_to_ids(text_tokens)
    # print("text_tokens: ", text_tokens, " , len(text_tokens)", len(text_tokens))
    # print("input_ids: ", input_ids, " , len(input_ids)", len(input_ids))
