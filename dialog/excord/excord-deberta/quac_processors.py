...
MULTI_SEP_TOKENS_TOKENIZERS_SET = {"deberta", "camembert", "bart", "mpnet"}
...
class QuacProcessor(DataProcessor):
    ...
    def _get_example_from_tensor_dict(self, tensor_dict, evaluate=False):
        ...
        # qas_id = str(tensor_dict["id"].numpy().decode("utf-8"))
        # question_text = str(tensor_dict["question"].numpy().decode("utf-8"))
        # context_text = str(tensor_dict["context"].numpy().decode("utf-8"))
        # answer_text = str(answer)
        # rewrite_text = str(tensor_dict["rewrite"].numpy().decode("utf-8"))
        # start_position_character = str(answer_start)
        # title = str(tensor_dict["title"].numpy().decode("utf-8"))
        # answers = str(answers)
    ...
    def _concat_history(self, qas, qa_idx, max_history, question_key="question"):
        ...
        if self.tokenizer_name in ["DebertaTokenizer", "DebertaTokenizerFast"]:
            sep_token += self.sep_token
...
def quac_convert_example_to_features(
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        ...
        # print("Encode dict : ")
        # print(" pairs : ",type(pairs), pairs)
        # print(" texts : ",type(texts), texts)
        # print(" truncation : ",type(truncation), truncation)
        # print(" padding_strategy : ",type(padding_strategy) , padding_strategy)
        # print(" max_seq_length : ",type(max_seq_length), max_seq_length)
        # print(" stride : ",type(max_seq_length - doc_stride - len(truncated_query) - sequence_pair_added_tokens), max_seq_length - doc_stride - len(truncated_query) - sequence_pair_added_tokens)
 
        encoded_dict = tokenizer.encode_plus(  # TODO(thom) update this logic
            texts,
            pairs,
            truncation=truncation,
            padding=str(padding_strategy),
...
def quac_convert_examples_to_features(
    ...
    with Pool(threads, initializer=quac_convert_example_to_features_init, initargs=(tokenizer,)) as p:
        ...
        # print("p.imap : ", p.imap(annotate_, examples, chunksize=32))
        print("annotate_ : ", annotate_)
