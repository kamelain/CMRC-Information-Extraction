...
if __name__ == "__main__":
  ...
  # "qid": ["C_ec865aa8cf664d4d879ed364dd7048ed_1_q#0"

  dia_id = pred_idx['qid'][0].split("_q#")[0]
  
  for line in open(args.model_output, 'r'):
    if line.strip():
      pred_idx = json.loads(line.strip())     # "qid": ["C_ec865aa8cf664d4d879ed364dd7048ed_1_q#0"
      dia_id = pred_idx.split("_q#")[0]       # C_ec865aa8cf664d4d879ed364dd7048ed_1
      turn_id = pred_idx.split("_q#")[1]      # 0
      for key, i in pred_idx.items():
        dia_id = key.split("_q#")
        print("dia_id", dia_id)
        preds[dia_id][key] = i, "x", "n"
        total += 1

      # for qid, qspan, qyesno, qfollowup in zip(pred_idx['qid'], pred_idx['best_span_str'], pred_idx['yesno'], pred_idx['followup']):
      #   preds[dia_id][qid] = qspan, "x", "n"
      #   total += 1
  for p in val:
    for par in p['paragraphs']:
      did = par['id']
